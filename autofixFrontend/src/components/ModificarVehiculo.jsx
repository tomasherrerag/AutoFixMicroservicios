...
const ModificarVehiculo = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/autofix/vehiculos/${id}`)
      .then(response => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:8080/autofix/vehiculos/${id}`, vehiculo)
      .then(response => {
  ...
  return (
    <StyledContainer>
      <Header>
        <BackButton variant="contained" onClick={() => navigate('/listar-vehiculos')}>
          Volver
        </BackButton>
        <StyledSubtitle>home &gt; menú vehículos &gt; listar vehículos &gt; modificar vehículo</StyledSubtitle>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3, width: '100%', maxWidth: 600 }}>
        ...
        <Box display="flex" justifyContent="space-between" mt={2}>
          ...
          <Button variant="contained" color="secondary" onClick={() => navigate('/listar-vehiculos')}>
            Volver
          </Button>
        </Box>
      </Box>
